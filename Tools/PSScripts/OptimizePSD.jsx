...
function hasLayerEffect(layer) {
  try {
        ...
        if (effectsDesc.hasKey(effectID)) {
          try {
            ...
            if (effectValueType === DescValueType.OBJECTTYPE) {
              ...
              for (
                var listIndex = 0;
                listIndex < effectList.count;
                listIndex++
              ) {
                try {
                  if (
                    effectList.getType(listIndex) ===
    ...
    var hasUserMask = getBooleanIfPresent(
      desc,
      app.stringIDToTypeID("hasUserMask"),
    );
    ...
    var hasVectorMask = getBooleanIfPresent(
      desc,
      app.stringIDToTypeID("hasVectorMask"),
    );
    ...
    for (var i = layers.length - 1; i >= 0; i--) {
      ...
      try {
        ...
        if (isGroup) {
          ...
          if (hasExplicitImageMainTag(layerName)) {
            if (rasterizeGroup(layer)) {
              ...
              stats.errors.push(
                "组(显式.img)合并失败: " + layerName + "，尝试处理子图层",
              );
        ...
        if (isText) {
          ...
          if (hasExplicitImageMainTag(layerName)) {
            if (rasterizeLayerWithEffects(layer)) stats.textConverted++;
            else {
...
function formatStats(stats, duration, savedFile) {
  ...
  if (stats.groupWithEffectMerged > 0) {
    message +=
      "📁✨ 组+特效 → 合并为单图: " + stats.groupWithEffectMerged + "\n";
  }
...
function processAndExport() {
  ...
  var statusText = progressWin.add(
    "statictext",
    [0, 0, 450, 20],
    "正在初始化...",
  );
